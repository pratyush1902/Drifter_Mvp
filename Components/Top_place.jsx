    ...
    <div className="pt-12 px-4 md:px-0">
      ...
      {popular.length > 0 ? (
        <>
          ...
          <div className="hidden md:flex justify-center flex-wrap gap-6 p-4 bg-white rounded-box">
            {popular.map((place) => {
              const imageUrl = place.attributes.Images?.data[0]?.attributes?.url;
              return (
                <div
                  key={place.id}
                  className="carousel-item relative group rounded-lg overflow-hidden shadow-lg max-w-xs"
                >
                  <img
                    src={
                      imageUrl
          ...
          <div className="md:hidden flex flex-col items-center gap-6">
            {popular.map((place) => {
              ...
              return (
                <div
                  key={place.id}
                  className="relative group rounded-lg overflow-hidden shadow-lg max-w-xs w-full"
                >
                  <img
                    src={
                      imageUrl
